// EliServer.kt — Ktor backend for Eli media downloader app
// Deploy on Railway.app (free tier)
//
// Gradle dependencies (server/build.gradle.kts):
...
fun Application.eliServer() {
    ...
    routing {
        ...
        get("/categories") {
            call.respond(listOf(
                Category("1","music", listOf("pop","rock","jazz","electronic","classical")),
                Category("2","art",   listOf("painting","sculpture","digital","photography")),
                Category("3","cook",  listOf("recipes","techniques","reviews","vegan")),
                Category("4","design",listOf("ui","graphic","interior","fashion"))
            ))
        }
        ...
        post("/messages") {
            ...
            if (!users.containsKey(req.senderId)) {
                call.respond(HttpStatusCode.NotFound, ErrorResponse("Sender not found"))
                return@post
            }
            val msg = EliMessage(UUID.randomUUID().toString(),
                req.chatId, req.senderId, req.text, Instant.now().toString())
...
@Serializable data class EliMessage(val id: String, val chatId: String,
                                     val senderId: String, val text: String, val createdAt: String)
